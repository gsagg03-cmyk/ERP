...
@section('title', 'খরচ সম্পাদনা করুন')

@section('content')
<div class="min-h-screen w-full px-2 sm:px-4 lg:px-6">
    <div class="mb-4 sm:mb-6">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">খরচ সম্পাদনা করুন</h1>
    </div>
        ...
        <form method="POST" action="{{ route('owner.expenses.update', $expense) }}">
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" 
                          id="notes" 
                          rows="3"
                          class="shadow border rounded w-full py-2 px-3 text-sm sm:text-base text-gray-700 @error('notes') border-red-500 @enderror">{{ old('notes', $expense->notes) }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            </div>
