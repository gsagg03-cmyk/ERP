...
<div class="min-h-screen w-full px-2 sm:px-4 lg:px-6">
    <div class="mb-4 sm:mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <a href="{{ route('manager.dashboard') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded text-sm sm:text-base">
            ড্যাশবোর্ডে ফিরুন
        </a>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-4 sm:p-6 mb-4 sm:mb-6">
        <form method="GET" action="{{ route('manager.due-customers') }}" class="flex gap-3">
            <div class="flex-1">
                <input type="text" 
                       name="search" 
                    ...
                    <tr class="{{ $sale->expected_clear_date && $sale->expected_clear_date->isPast() ? 'bg-red-50' : 'hover:bg-gray-50' }}">
                        ...
                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm">
                            <div class="flex flex-col gap-1">
                                <a href="{{ route('manager.voucher.print', $sale->id) }}" target="_blank" class="text-blue-600 hover:text-blue-900 font-mono">
                                    {{ $sale->voucher_number ?? 'N/A' }}
                                </a>
                                @if($sale->profitRealizations->count() > 0)
                                    <div class="text-xs text-gray-500">
                                        @foreach($sale->profitRealizations as $pr)
                                            <a href="{{ route('manager.payment.voucher', $pr->id) }}" target="_blank" class="text-green-600 hover:text-green-900 block">
                        ...
                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm">
                            <a href="{{ route('manager.payment.record', $sale->id) }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-3 rounded text-xs">
                                টাকা নিন
                            </a>
                        </td>
                    </tr>
